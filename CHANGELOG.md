...
- **一键卸下身份徽章** — 工具页新增快捷按钮，可一次清空头像下方的三个挑战徽章展示位，让身份卡片恢复简洁样式。
- **隐藏琴女 Icon** — 设置页新增入口图标开关，可隐藏 Play 按钮旁的 Sona 头像；隐藏后仍可通过已配置的面板快捷键呼出。

### ✨ 改进

- **排位补位保护** — 秒选英雄现在会综合选人补位标记、分配原因和排队分路偏好识别补位；确认补位后跳过自动预选与锁定，避免误选原分路英雄。
- **游戏模式布局** — 玩家对战的模式卡片改为按当前可见数量等距排列，整组保持居中，并在左右两侧保留自然留白。
- **资源名称智能导入** — 自定义资源支持省略文件后缀；完整路径加载失败时也会自动尝试常见图片与视频后缀，并为失效的历史资源显示明确提示。

### 🐛 Bug 修复

- **自动 Ban 首次触发** — 修复首次进入 Ban 阶段时本地 Ban 操作尚未生成，导致必须先点击任意英雄才会触发自动 Ban 的问题；现在会持续等待阶段与英雄列表就绪后自动完成禁用。
- **海斗数据源适配** — 适配 ARAM.GG 新版 JSON 数据接口，恢复单英雄海克斯推荐、总体胜率与 T 级；插件启动时异步缓存全英雄海斗榜，选人角标不再复用普通大乱斗数据。
